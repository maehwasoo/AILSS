...
import {
	formatPythonAgentResult,
	formatPythonRetrieveResult,
} from "./pythonApi/resultFormatting.js";
import type { AilssPythonApiServiceStatusSnapshot } from "./pythonApi/pythonApiServiceTypes.js";
import {
	createAutoIndexScheduler,
	createIndexerRunner,
...
import { getVaultPath } from "./utils/pluginPaths.js";
...
export default class AilssObsidianPlugin extends Plugin {
	...
	private readonly mcpHttpService = createMcpHttpServiceController({
		app: this.app,
		...
		getMcpUrl: () => this.getMcpHttpServiceUrl(),
		onMcpStatusChanged: () => {
			if (!this.mcpStatusBarEl) return;
			renderMcpStatusBar(this.mcpStatusBarEl, this.getMcpHttpServiceStatusSnapshot());
		},
		onPythonStatusChanged: () => undefined,
		onIndexerSnapshot: () => undefined,
	});

	private readonly pythonApiService = createPythonApiServiceController({
		app: this.app,
		pluginId: this.manifest.id,
		getSettings: () => this.settings,
		saveSettings: async () => {
		...
		getPythonUrl: () => this.getPythonApiServiceUrl(),
		onMcpStatusChanged: () => undefined,
		onPythonStatusChanged: () => {
	...
	private readonly indexer = createIndexerRunner({
		app: this.app,
		pluginId: this.manifest.id,
		getSettings: () => this.settings,
		saveSettings: async () => {
		...
		onMcpStatusChanged: () => undefined,
		onPythonStatusChanged: () => undefined,
		onIndexerSnapshot: (snapshot) => {
			if (!this.statusBarEl) return;
			renderIndexerStatusBar(this.statusBarEl, snapshot);
		},
	});

	private readonly autoIndex = createAutoIndexScheduler({
		getSettings: () => this.settings,
