...
export function promptTemplate(kind: PromptKind): string {
	const promptDoc = promptFilename(kind);

	const body = [
		"# AILSS Obsidian Vault 작업 규칙",
		"",
		...
		"- 단일 문헌/페이지 근거는 Fetch(페치, fetch)로 원문 확보해요. 다만 Obsidian 내부 지식은 AILSS MCP(에일스 MCP)로 먼저 질의해요.",
		...
		"- AILSS MCP 도구 사용은 강제해요: 요약·분류·리뷰 작업 전 반드시 MCP 질의로 원문/메타를 조회해요.",
		"",
		...
		"- `status`는 draft, in-review, active, archived 중에서 현재 상태를 표시해요.",
		...
		"4) 문자열 재확인(string search): (가능하면) `search_vault`로 실제 문서와 줄 번호를 확인해요.",
		...
		"8) 검증(validation): 아래 4.4 커버리지 체크리스트를 기준으로 누락을 점검해요.",
		"",
		...
		"- 폴더 첫 문서는 허브 노트(hub note)로 만들고 파일명은 폴더명과 동일하게 해요.",
		"  - 프론트매터 권장: `entity: hub`, `layer: logical`, `instance_of: ['[[hub]]']`, `part_of: ['[[상위 폴더명]]']`",
		...
		`- {{PROMPT_FILE}} 문서에서는 표시 텍스트(display text)로 제목을 명시해요. 항상 [[경로|제목]] 형식을 사용해 경로 대신 제목만 보이게 해요. 예: [[20. Areas/30. SOPT/코드 리뷰/PROMPT|PROMPT]], [[20. Areas/70. Claude Code/Commands/write-pr|write-pr]].`,
		...
		"- 의무(Required): 요약(summarize), 분류(classify), 리뷰(review), 링크 점검 전 반드시 AILSS MCP(에일스 MCP)로 볼트 메타를 조회해요.",
		"- 원칙:",
		"  - 읽기 우선(read-only)로 사용해요. 쓰기 작업은 별도 승인 후 진행해요.",
		"  - 선행 질의: `get_context`(의미 검색), `search_vault`(문자열 검색), `get_vault_tree`(구조/경로 확인)",
		"  - 후속 질의: `read_note`(본문/프론트매터 조회), `get_typed_links`(타입드 링크 확장)",
		"- 권장 흐름(flow):",
		"  1) `get_context`로 후보 문서를 수집해요.",
		"  2) `read_note`로 프론트매터와 본문을 확인해요.",
		"  3) 문자열 근거가 필요하면 `search_vault`로 줄 번호를 확보해요.",
		"  4) 타입 링크 후보를 검토하고(4장 체크리스트), 필요 시 프론트매터에 반영해요.",
		"  5) 링크·자산 경로 점검을 수행해요.",
		"- 실패 대응: MCP 호출 실패 시 오류와 사유를 기록하고, 임시로 `rg`/`find`로 대체해요.",
		"",
		"### 12.1 AILSS MCP 도구 요약",
		"- `get_context`: 의미론적 검색(semantic search)으로 유사 맥락을 찾을 때 사용해요.",
		"- `search_vault`: 전역 문자열 검색(global text search)으로 줄 번호/정확한 위치가 필요할 때 사용해요.",
		"- `read_note`: 특정 문서를 전체 맥락으로 검토해야 할 때 호출해요.",
		"- `get_typed_links`: 타입드 링크(typed link)로 연결된 노트를 incoming/outgoing으로 확장해요.",
		"- `get_vault_tree`: 폴더/파일 구조(tree)를 확인해 경로 추측을 줄여요.",
		"- `frontmatter_validate`: 프론트매터 필수 키 존재 여부 + id/created 일치 여부를 점검해요.",
		"",
		"### 12.2 의미론적 검색(semantic search) 지침",
		"- **강한 지침(strong directive)**: 모든 조사·편집 의사결정은 `get_context` 실행으로 시작해요.",
		"- 기본 규칙: 구조 검색(`search_vault`)과 의미 검색(`get_context`)을 항상 짝지어 실행해 문자 증거와 문맥 증거를 동시에 확보해요.",
		"- 결정 전 검증: 의미 검색과 문자열 검색 결과가 모순되면 `read_note`로 원문을 확인한 뒤 판단 이유를 기록해요.",
		"",
		"## 13. 점검 체크리스트",
		'- 작업 전: 관련 노트·자산 목록 확보(`get_context`, `search_vault`, `rg "assets/" -n`)',
		"- 프론트매터: 필수 키(id, created, title, entity, layer, status) 확인",
		"- 타입 링크: 4.3 체크리스트 항목(instance_of, part_of, depends_on, uses, implements, cites, same_as, supersedes) 전부 검토",
