...
- Typed-link keys (only include when non-empty; omit key when you have no values): `instance_of`, `part_of`, `depends_on`, `uses`, `implements`, `cites`, `summarizes`, `derived_from`, `explains`, `supports`, `contradicts`, `verifies`, `blocks`, `mitigates`, `measures`, `authored_by`, `supersedes`, `same_as`
...
- External vs internal sources:
  ...
  - Use `summarizes` / `derived_from` / `explains` / `supports` / `contradicts` / `verifies` / `blocks` / `mitigates` / `measures` for non-citation note-to-note relations
